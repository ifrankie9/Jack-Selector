...
#*                                                                         *
        ...
        self.directionComboBox.addItem("Tension and Compression")
...
#*                                                                         *
        ...
        self.assetModel.setTable("Result")
        ...
        self.assetView.setColumnWidth(1, 70)
        ...
        self.logModel.setTable("Selection")
        ...
        logLabel = QLabel("Model &Details   &nbsp;&nbsp;&nbsp;&nbsp;"
                          "<font color=#ff4d4d size=1>"
                          "*Please select before print")
...
#*                                                                         *
        ...
        fileNewAction = self.createAction("&New...", self.fileNew,
                QKeySequence.New, "filenew", "Create New Selection")
        fileOpenAction = self.createAction("&Open...", self.fileOpen,
                QKeySequence.Open, "fileopen",
                "Open An Existing Selection File")
        fileSaveAction = self.createAction("&Save", self.fileSave,
                QKeySequence.Save, "filesave", "Save Selection")
        fileSaveAsAction = self.createAction("Save &As...",
                self.fileSaveAs, "", "filesaveas",
                tip="Save Selection Using A New Name")
        filePrintAction = self.createAction("&Print", self.filePrint,
                QKeySequence.Print, "fileprint", "Print Report")
        
        duffnortonAction = self.createAction("&Duff-Norton Page",
        ...
        quoteAction = self.createAction("&Request a Quote",
                self.requestQuote, "", "send", "Request a Quote")
        ...
        helpHelpAction = self.createAction("&Help", self.helpHelp,
                QKeySequence.HelpContents, "question", "Find Answers Here")
        ...
        self.addActions(helpMenu, (quoteAction, literatureAction,
                                   modelAction, duffnortonAction,
                                   None, helpAboutAction, helpHelpAction))
...
#*                                                                         *
    ...
    def calculate(self):
        ...
        for i in range(row): 
            ...
            query.exec_("UPDATE Result SET torqueperjack"
                        " = %.2f WHERE id = %d" % (torqueCalc, i+1))
            query.exec_("UPDATE Result SET torquetotal"
                        " = %.2f WHERE id = %d" % (totalTorqueCalc, i+1))

            query.exec_("UPDATE Result SET hptotal"
                        " = %.2f WHERE id = %d" % (totalHpCalc, i+1))

            if  0 <= float(hpCalc) < float(horsePower):
                query.exec_("UPDATE Result SET hpperjack"
            ...
            if loadmax > 70000:
                if travelinputrpm < 900:
                    query.exec_("UPDATE Result SET travelrate"
                                " = %.1f WHERE id = %d" % \
                                (travelinputrpm, i+1))
                else:
                    query.exec_("UPDATE Result SET travelrate"
            ...
            query.exec_("UPDATE Result SET dutycycle"
                        " = %.1f WHERE id = %d" % (dutyCycle, i+1))

            if float(maxInPerHour) > float(inputduty):
                query.exec_("UPDATE Result SET maxinperhour"
...
#*                             ITEM   FUNCTION                             *
    ...
    def showEndCombo(self):
        name = str(self.directionComboBox.currentText())
        endType = self.translatingComboBox.currentText()
        if name == "Tension and Compression":
...
#*                                                                         *
    ...
    def updateFileMenu(self):
        ...
        if recentFiles:
            ...
            for i, fname in enumerate(recentFiles):
                action = QAction(QIcon(":/duff.ico"), "&%d %s" % (
    ...
    def updateSelector(self):
        ...
        self.selector.append(str(self.dutyComboBox.currentText()))
        self.selector.append(str(self.checklock))
        self.selector.append(str(self.checkanti))
...
#*                                                                         *
        ...
        if fname:
            ...
            if self.filename:
                ...
                if self.selector[4] in ("Upright", "Inverted"):
                    ...
                    self.radialLabel.show()
                    self.radialReadLabel.show()
                elif self.selector[4] == "Double Clevis":
                    self.translatingComboBox.hide()
                    self.translatingLabel.hide()
                    self.keyoptionLabel.hide()
                    self.keyoptionComboBox.hide()
                ...
                if self.selector[15] == "True":
                    self.selflocking.setChecked(True)
                elif self.selector[15] == "False":
                    self.selflocking.setChecked(False)
                if self.selector[16] == "True":
                    self.antibacklash.setChecked(True)
                elif self.selector[16] == "False":
                    self.antibacklash.setChecked(False)
    ...
    def fileSave(self):
        ...
        if self.filename is None:
            ...
            try:
                ...
                fh.write(u"%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,"
                         "%s,%s,%s" %\
                         (arran, dyn, sta, jacktype, config, trans,
                          keyed, bear, direction, endfix, travel, speed,
                          speedunit, duty, dutyunit, checklock, checkanti))
    ...
    def filePrint(self):
        ...
        html += ("<h1 align=left>Jack  Selection Report"
                 "<img src=':/logo_36.png' width='274' height='132' "
                 "align=right></h1>")
        ...
        html += ("<p><table width=100% cellpadding=2 cellspacing=2>"
                 "<font size=4><b>POWER REQUIREMENTS</b></font>")
        ...
        html += ("<p><font size=4>Should you have any further"
                 ...
                 " in our products.<br>"
                 "</font></p>")
        html += ("<h1 align=left>Selection  Tips"
                 "<img src=':/logo_361.png' width='277' height='36'"
                 "align=right></h1>")

        html += ("<h5 align=left>"
                 "<p>&nbsp;</p>"
                 "<p align=right><font size=4>%s</font><br>"
                 "<font size=4><b>www.duffnorton.com</b>"
                 "</font></p></h5>") % (date)

        html += ("<h2><font color=#002db3>"
                 "&diams; Quick Notes</h2>")
        html += ("<table border=1 font size=4 width=60%><tr><th>Speed "
                 "<font color=red>&uarr;</font><font color=green>&darr;"
                 "</font></th><td rowspan=3 valign=middle align=center>"
                 "&rArr;</td>&nbsp;<td rowspan=3 valign=middle align=center>"
                 "<b>Actuator Size <font color=red>&uarr;</font>"
                 "<font color=green>&darr;</font></b></td></tr><tr><th>"
                 "Duty <font color=red>&uarr;</font>"
                 "<font color=green>&darr;</font></th></tr><tr><th>"
                 "Lifting Screw Diameter <font color=red>&uarr;</font>"
                 "<font color=green>&darr;</font></th></tr><tr><th>"
                 "Guided Load <font color=red>&#x2713;</font></th>"
                 "<td rowspan=2 valign=middle align=center>&rArr;</td>&nbsp;"
                 "<td rowspan=2 valign=middle align=center><b>Actuator Size"
                 "<font color=red>&darr;</font></b></td></tr><tr><th>"
                 "Tension Load Only"
                 "<font color=red>&#10003;</font></th></tr></table>")

        html += ("<h2><font color=#002db3>"
                 "&diams; What can I do to reduce the size of the "
                 "actuator result?</h2>")
        html += ("<p><font size=4>&nbsp; 1. &nbsp; Reduce the Travel Rate "
                 "(speed)<br>"
                 "&nbsp; 2. &nbsp; Reduce the Duty<br>"
                 "&nbsp; 3. &nbsp; For High Duty running, "
                 "Switch to Ball Screw<br>"
                 "&nbsp; 4. &nbsp; For Long Column Length required, "
                 "Guide the load<br>"
                 "&nbsp; 5. &nbsp; Re-Design for a Tension Load to"
                 " eliminate the Column Strength</p>")
        
        html += ("<h2><font color=#002db3>"
                 "&diams; Frequently Asked Questions</h2>")
        html += ("<p><font size=4><b>&nbsp; &#8680;&nbsp;What is the cause "
                 "of thermal or heat build-up in an actuator unit?</b><br>"
                 "<font size=4>&nbsp;&nbsp;&nbsp;The duty cycle, the length of the screw, "
                 "the magnitude of the load, and the efficiency of the "
                 "actuator unit all have a direct influence on the amount "
                 "of heat generated within the actuator model. Since most "
                 "of the power input is used to overcome friction, a large "
                 "amount of heat is generated in the worm gear set in both "
                 "ball screw and machine screw actuator models, and in the "
                 "lifting screw of machine screw actuator units. Long lifts "
                 "can cause serious overheating.</p>")
        html += ("<p><font size=4><b>&nbsp; &#8680;&nbsp;What is the life "
                 "of the worm gear actuator?</b><br>"
                 "<font size=4>&nbsp;&nbsp;&nbsp;The life of a machine "
                 "screw actuator screw, nut and worm gear set varies "
                 "considerably due to extent of lubrication, abrasive "
                 "or chemical action, overloading, eccentric loading, "
                 "excessive heat, improper maintenance, etc.</p>")
        html += ("<p><font size=4><b>&nbsp; &#8680;&nbsp;Why is it ever "
                 "necessary to use a keyed lifting screw?</font></b><br>"
                 "<font size=4>&nbsp;&nbsp;&nbsp;When an actuator unit "
                 "is operated, the rotation of the worm shaft causes the "
                 "worm gear to rotate. The worm gear is threaded to "
                 "accommodate the lifting screw thread; as the worm "
                 "gear turns, the friction forces on the screw thread "
                 "act to turn the screw also. The greater the load on "
                 "the actuator unit, the greater the tendency of the "
                 "screw to turn. It is obvious that if the screw turns "
                 "with the nut (worm gear), it will not raise the load."
                 " In those cases where a single unit is used, and where"
                 " the load cannot be restrained from turning, it is "
                 "necessary to key the lifting screw.</p>")
...
def main():
    ...
    app.setWindowIcon(QIcon(":/duff.ico"))
